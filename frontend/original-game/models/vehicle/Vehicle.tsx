...
>;
type DerivedWheelInfo = WheelInfo &
  Required<
...
export function Vehicle({
  ...
  const [chassisBody, vehicleConfig, wheelInfo, wheels] = useStore((s) => [
    s.chassisBody,
    s.vehicleConfig,
    s.wheelInfo,
    s.wheels,
  ]);
  const { back, force, front, height, maxBrake, steer, maxSpeed, width } =
    vehicleConfig;
  ...
  useLayoutEffect(
    () => api.sliding.subscribe((sliding) => (mutation.sliding = sliding)),
    [api],
  );
    ...
    if (!editor) {
      ...
      defaultCamera.rotation.z = lerp(
        ...
        (camera !== "BIRD_EYE" ? 0 : Math.PI) +
          (-steeringValue * speed) / (camera === "DEFAULT" ? 40 : 60),
        delta,
      );
    }
